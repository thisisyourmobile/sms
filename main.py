...
# Main loop with 1 hour running and 30 minutes rest
while True:
    # Calculate start and end times for the active period
    start_time = datetime.now()
    end_time = start_time + timedelta(hours=1)
    
    print(f"\n{Fore.GREEN}Starting 1-hour active period at {start_time.strftime('%H:%M:%S')}{Style.RESET_ALL}")
    ...
    print(f"\n{Fore.YELLOW}Starting 30-minute rest period at {rest_start.strftime('%H:%M:%S')}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Script will resume at {rest_end.strftime('%H:%M:%S')}{Style.RESET_ALL}")
    
    # Countdown during rest period
    while datetime.now() < rest_end:
        remaining = rest_end - datetime.now()
        hours, remainder = divmod(remaining.seconds, 3600)
        minutes, seconds = divmod(remainder, 60)
        print(f"{Fore.YELLOW}Resting... {minutes:02d}:{seconds:02d} remaining", end="\r")
        time.sleep(1)
    
    print(f"\n{Fore.GREEN}Rest period complete. Resuming operations.{Style.RESET_ALL}")
